...
// Intercept password recovery links before React hydrates
...
function AppContent({ Component, pageProps, router }: AppPropsWithRouter) {
  // Track user activity so the backend session stays alive
  useActivityTracker()

  return (
    <>
      <Component {...pageProps} />
      {/* SessionTimeoutWarning internally runs useSessionMonitor */}
...
export default function App({ Component, pageProps, router }: AppPropsWithRouter) {
  ...
  useEffect(() => {
    ...
    if (hash.includes("access_token") && hash.includes("token_type=bearer")) {
      ...
      // Regular magic link — redirect to markets once
      if (!sessionStorage.getItem("magiclink_redirected")) {
  ...
  useEffect(() => {
    if (router.pathname !== "/auth/reset") {
      if (sessionStorage.getItem("password_reset_active") && !window.location.href.includes("access_token")) {
